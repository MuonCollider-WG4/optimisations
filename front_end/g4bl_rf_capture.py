...
class RFCapture:
    ...
    def build_line(self, line):
        ...
        for i in range(line["n_rf_repeats"]):
            ...
            print("\nSimulating cell", i, "of", line["name"])
...
def get_beam_file_full(my_linac, rf_capture, t_range, n_particles, source):
    beam_def = {
        "filename":"beam.txt",
        "out_dir":os.path.split(my_linac.lattice_filename)[0],
...
class Analysis:
    def __init__(self, base_dir):
        ...
        self.transmission_region = "buncher" # check for transmission to end of this region for first transmission check
        self.acceptance_region = "continuation" # name of the acceptance cells
        self.weight_context_dict = {}

    def load_data(self, my_linac, rf_capture, plot_dir):
        self.weight_context_dict = {"none":xboa.core.WeightContext()}
        xboa.core.Hitcore.set_weight_context(xboa.core.WeightContext())
        ...
        for bunch in self.bunch_list:
            ...
            if old_bunch:
                for hit in bunch:
                    ...
                    if len(old_hits) == 0:
                        print("new event", hit["event_number"], hit["particle_number"], hit["pid"]) # maybe decays are on?
                    elif old_hits[0][self.weight_type] == 0.0: # if weight_type is local, we make it sticky for every plane after the failing one
                        ...
                        hit[self.weight_type] = 0.0
            old_bunch = bunch
        print(f"    ... {bucket_switchers} bucket switchers")

    def check_transmission(self, n_min, target_cell_name):
        """
        Iterate through bunch list; apply a transmission cut between:
        * first bunch having more than n_min events
        * first bunch sitting in a cell with name like target_cell_name
        """
    ...
    def init_hist(self, n_min):
        ...
        for name, context in self.weight_context_dict.items():
            axes.clear()
            xboa.core.Hitcore.set_weight_context(context)
            ...
            if beam["type"] == "longitudinal_grid":
                ...
                t_bins = [150.0 + i for i in range(101)]
                e_bins = [10.0 * i for i in range(101)]
            ...
            print("init_hist plotting ", name, len(t_list), "/", len(bunch), "hits")
            h_dict[name] = axes.hist2d(t_list, e_list, [t_bins, e_bins])
            #h_all, xedges, yedges = axes.hist2d(t_list_all, e_list_all, [t_bins, e_bins])

            axes.set_xlabel("t [ns]")
            axes.set_ylabel("KE [MeV]")
            axes.text(1.01, 1.05, name, transform=axes.transAxes, horizontalalignment="right")
            axes.set_title(f"N survived: {len(t_list)} / {len(self.bunch_list[0])}")
            figure.savefig(f"{self.plot_dir}/a_init_t_vs_e_{name}.png")

        axes.clear()
        axes.set_xlabel("t [ns]")
        axes.set_ylabel("KE [MeV]")
        axes.set_title(f"Capture probability")
        xboa.common.matplotlib_wrapper.plot_hist2d_ratio(axes, h_dict["all cuts"][0], h_dict["transmitted"][0], t_bins, e_bins, on_nan=float('nan'), hist_args={})
        figure.savefig(f"{self.plot_dir}/a_init_t_vs_e_capture_probability.png")

        axes.clear()
        axes.set_xlabel("t [ns]")
        axes.set_ylabel("KE [MeV]")
        axes.set_title(f"Survival probability")
        xboa.common.matplotlib_wrapper.plot_hist2d_ratio(axes, h_dict["all cuts"][0], h_dict["is not reference"][0], t_bins, e_bins, on_nan=float('nan'), hist_args={})
        figure.savefig(f"{self.plot_dir}/a_init_t_vs_e_survival_probability.png")

    def get_acceptance(self, beam_file_n_protons):
        n_min = 5
        self.bunch_list[0][0]['global_weight'] = 0
        self.bunch_list[0][1]['global_weight'] = 0
        self.weight_context_dict["is not reference"] = xboa.core.Hitcore.get_weight_context()
        self.weight_context_dict["is reference"] = ~(xboa.core.Hitcore.get_weight_context())
        xboa.core.Hitcore.set_weight_context(xboa.core.WeightContext())
        self.check_containment(self.acceptance_region) # apply CUTS
        self.weight_context_dict["in rf bucket"] = xboa.core.Hitcore.get_weight_context()*self.weight_context_dict["is not reference"]
        xboa.core.Hitcore.set_weight_context(xboa.core.WeightContext())
        self.check_transmission(n_min, self.acceptance_region)
        self.weight_context_dict["transmitted"] = xboa.core.Hitcore.get_weight_context()*self.weight_context_dict["is not reference"]
        xboa.core.Hitcore.set_weight_context(xboa.core.WeightContext())
        self.weight_context_dict["all cuts"] = self.weight_context_dict["transmitted"]*self.weight_context_dict["in rf bucket"]
        xboa.core.Hitcore.set_weight_context(self.weight_context_dict["all cuts"])

        n_transported =  self.bunch_list[-1].bunch_weight() # number of mu+ transported
        xboa.core.Hitcore.set_weight_context(xboa.core.WeightContext())

        # calculate the normalisation
        beam = self.my_linac.beam
        beam_source = beam["beams"][0]["file_in"]
        beam_np = beam["beams"][0]["n_particles"]
        beam_in_full = xboa.bunch.Bunch.new_from_read_builtin("g4beamline_bl_track_file", beam_source)
        n_secondaries = beam_in_full.bunch_weight()
        proportion_simulated = beam_np/n_secondaries

        n_transported_per_pot = n_transported/(beam_np/n_secondaries)/beam_file_n_protons
        print(f"Transported {n_transported} muons from {beam_np} secondaries.")
        print(f"Target simulation yielded {n_secondaries} secondaries from {beam_file_n_protons} protons")
        print(f"Fractional yield was {n_transported_per_pot} mu+ per proton on target")

        json_transport = {
            "n_secondaries_full_beam":n_secondaries,
            "n_secondaries_simulated":beam_np,
            "n_mu+_captured_and_transported":n_transported,
            "beam_file_n_protons":beam_file_n_protons,
            "fractional_yield_of_mu+_per_proton_on_target":n_transported_per_pot
        }
        init_bunch = self.init_bunch(n_min)
        for name, context in self.weight_context_dict.items():
            xboa.core.Hitcore.set_weight_context(context)
            weight = init_bunch.bunch_weight()
            json_transport["number "+name] = weight
            xboa.core.Hitcore.set_weight_context(xboa.core.WeightContext())
        fout = open(f"{self.plot_dir}/performance.json", "w")
        fout.write(json.dumps(json_transport))
        fout.close()

    def get_bucket_list(self, bunch):
        bucket_list = []
        final_ev_list = [hit["event_number"] for hit in self.bunch_list[-1]]
        final_bucket_list = [hit["proper_time"] for hit in self.bunch_list[-1]]
        for hit in bunch:
            if hit["weight"] < 0.5:
                continue
            try:
                index = final_ev_list.index(hit["event_number"])
                bucket_list.append(final_bucket_list[index] % 5)
            except:
    ...
    def do_plots(self, do_movie=True):
        t_range = []
        e_range = [0]
        for bunch in self.bunch_list:
        ...
        figure_bucket = matplotlib.pyplot.figure("bucket")
        axes_bucket = figure_bucket.add_subplot(1, 1, 1)
        ...
        for bunch in self.bunch_list:
            ...
            axes_bucket.clear()
            axes_dt.clear()
            for weight_context, colour in [("is reference", "red"), ("is not reference", "grey"), ("transmitted", "magenta"), ("all cuts", "blue")]:
                xboa.core.Hitcore.set_weight_context(self.weight_context_dict[weight_context])
                ...
                axes.scatter(t_list[2:], e_list[2:], s=1, color=colour)
                axes_dt.scatter(dt_list[2:], e_list[2:], s=1, color=colour)
                if colour == "blue":
                    bucket_list = self.get_bucket_list(bunch)
                    axes_bucket.scatter(t_list, e_list, s=1, c=bucket_list, cmap="gnuplot")
            ...
            axes.scatter(t_list, e_list, s=4, edgecolors="blue", facecolors="black")
            axes_bucket.scatter(t_list, e_list, s=4, edgecolors="blue", facecolors="black")
            axes_dt.scatter(dt_list, e_list, s=4, edgecolors="blue", facecolors="black")
            ...
            for an_axes in axes, axes_dt, axes_bucket:
                an_axes.set_xlabel("t-t$_0$ [ns]")
                an_axes.set_ylabel("Kinetic Energy [MeV]")
                an_axes.plot(t_range, [self.target_ke, self.target_ke], linestyle="--")
                an_axes.set_ylim(e_range)
                an_axes.text(0.76, 0.55, text, transform=axes.transAxes)
            axes_dt.set_xlim(dt_range)
            axes.set_xlim(t_range)
            axes_bucket.set_xlim(t_range)
            figure.savefig(f"{self.plot_dir}/t_vs_e_z={int(z):0>6}.png")
            figure_dt.savefig(f"{self.plot_dir}/dt_vs_e_z={int(z):0>6}.png")
            figure_bucket.savefig(f"{self.plot_dir}/bucket_t_vs_e_z={int(z):0>6}.png")
        print("\n\nDone")

    def write_beam_file(self, start_of_lattice_name):
        """
        Write the beam file for the first bunch having lattice 'start_lattice_name' (string)
        """
        for bunch in self.bunch_list:
            cell = self.get_cell(bunch[0]["z"])
            if cell["name"] == start_of_lattice_name:
                filename = f"{self.base_dir}/beam_end_of_capture.txt"
                bunch.hit_write_builtin(
                    "g4beamline_bl_track_file", filename, user_comment=f"Beam generated from {self.base_dir}")
                print("Wrote", filename)
                return
        raise RuntimeError(f"Failed to find lattice {start_of_lattice_name} for beam file")
...
def build_longitudinal_capture(run_dir, do_execute, args):
    cleanup = do_execute
    ...
    get_beam_file_full(my_linac, rf_capture, [args["t_min"], args["t_max"]], args["n_events"], "output/mucol_target_v2/energy=5000;_end_length=2000_plots/beam_out.txt") # 58664
    my_linac.physics.do_stochastics = args["stochastics"]
    my_linac.max_z = max([sol["z_position"] for sol in my_linac.elements])
    my_linac.track_cuts.keep = args["keep"]
...
def run_one(args):
    ...
    my_linac, rf_capture = build_longitudinal_capture(run_dir, args["do_execute"], args)
    if args["do_execute"]: # alternatively we just redo the analysis
        my_execution = g4bl_interface.g4bl_interface.G4BLExecution(my_linac)
        my_execution.execute()
    if args["do_analysis"]: # alternatively we just redo the simulation
        my_analysis = Analysis(run_dir)
        my_analysis.load_data(my_linac, rf_capture, run_dir)
        my_analysis.get_acceptance(beam_file_n_protons=100000)
        my_analysis.do_plots(do_movie=args["do_movie"])
        my_analysis.write_beam_file("continuation")
    print()

def main():
    base_dir = f"output/rf_capture_v24/"
    config = {
        "n_events":5000, #58664,
        "keep":[-13, 211],
        "stochastics":1,
    ...
    #arg_list = [{"t_min":ti*5.0, "t_max":(ti+1)*5.0, } for ti in range(1)]
    arg_list = [{"t_min":0.0, "t_max":0.001}]
    for args in arg_list:
        config.update(args)
        config["run_dir"] = os.path.join(base_dir, dir_name(args, True))
        run_one(config)
